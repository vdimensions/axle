...
namespace Axle.Text.Formatting
{
    ...
    public abstract class AbstractCustomFormatter<T> : ICustomFormatter<T>
    {
        ...
        {
            sb.Replace(format, FormatValue(format, arg, formatProvider));
        }
        ...
        public string Format(string format, T arg, IFormatProvider formatProvider)
        {
            ...
            {
                result = FormatValue(format, arg, formatProvider);
            }
            return result;
        }
        protected abstract string FormatValue(string format, T arg, IFormatProvider formatProvider);
    }
        ...
        where TFP: class, IFormatProvider
    {
        ...
        public string Format(string formatString, T arg, TFP formatProvider)
        {
            ...
            {
                result = FormatValue(formatString, arg, formatProvider);
            }
            return result;
        }

        protected sealed override string FormatValue(string format, T arg, IFormatProvider formatProvider) => 
            FormatValue(format, arg, (TFP)formatProvider);
        protected abstract string FormatValue(string format, T arg, TFP formatProvider);
